...
class PackedDataset(IterableDataset):
    ...
    def split_buffer(buffer, max_tokens, img_start_token_id, img_token_id, img_end_token_id):
        ...
        def _get_video_start_img_idx_list(split_sizes):
            cum_idxs = torch.cumsum(split_sizes, dim=0) - split_sizes
            expanded_idxs = torch.repeat_interleave(cum_idxs, split_sizes)
            return expanded_idxs
        
        def _get_video_start_idx_list(split_sizes):
            idxs = torch.arange(split_sizes.size(0))
            expanded_idxs = torch.repeat_interleave(idxs, split_sizes)
            return expanded_idxs

        def _split(sample_to_split, left_idx, right_idx, left_img_idx, right_img_idx, left_video_idx, right_video_idx):
            ...
            for k in sample_to_split:
                ...
                elif k in ['split_sizes']:
                    left_sample[k] = sample_to_split[k][:left_video_idx]
                    if right_sample is not None:
                        right_sample[k] = sample_to_split[k][right_video_idx:]
                else:
        ...
        while buffer['input_ids'].size(0) > max_tokens:
            ...
            split_sizes_count = buffer['split_sizes'].sum().item()
            image_flags_count = buffer['image_flags'].sum().item()
            video_start_img_idx_list = _get_video_start_img_idx_list(buffer['split_sizes']) # index of img_start_idx_list, denote the start img of a video
            video_start_idx_list = _get_video_start_idx_list(buffer['split_sizes']) # index of img_start_idx_list, denote the current img in idx_th video
            assert len(img_start_idx_list) == len(img_end_idx_list) == split_sizes_count == image_flags_count, f"{img_start_idx_list=}, {split_sizes_count=}, {image_flags_count=}"
            assert len(video_start_idx_list) == len(video_start_img_idx_list) == len(img_start_idx_list)
            ...
            if _image_is_splitted(buffer['input_ids'], max_tokens):
                ...
                if buffer['input_ids'][max_tokens] == img_start_token_id:
                    cut_left_img_idx = video_start_img_idx_list[cut_idx]
                    cut_left_idx = img_start_idx_list[cut_left_img_idx]
                else:
                    cut_left_img_idx = video_start_img_idx_list[cut_idx - 1]
                    cut_left_idx = img_start_idx_list[cut_left_img_idx]
                    
                cut_right_idx = cut_left_idx
                cut_right_img_idx = cut_left_img_idx
                
                cut_left_video_idx = video_start_idx_list[cut_left_img_idx]
                cut_right_video_idx = cut_left_video_idx
            else:
                ...
                if cut_img_idx < len(img_start_idx_list):
                    ...
                    cut_right_video_idx = video_start_idx_list[cut_right_img_idx]
                    if cut_right_img_idx > 0:
                        assert video_start_idx_list[cut_right_img_idx] > video_start_idx_list[cut_right_img_idx-1], "video can be splitted"
                else:
                    cut_right_idx = None
                    cut_right_img_idx = None
                    cut_right_video_idx = None
                ...
                cut_left_video_idx = cut_right_video_idx if cut_right_video_idx is not None else buffer['split_sizes'].size(0)
            ...
            left, right = _split(
                ...
                left_video_idx=cut_left_video_idx,
                right_video_idx=cut_right_video_idx,
            )

            assert (left['input_ids'] == img_end_token_id).sum() == (left['input_ids'] == img_start_token_id).sum() == left['pixel_values'].size(0) == left['split_sizes'].sum().item(), f"{left['pixel_values'].size(0)=},{left['split_sizes'].sum().item()=}"
            if right is not None:
                assert (right['input_ids'] == img_end_token_id).sum() == (right['input_ids'] == img_start_token_id).sum() == right['pixel_values'].size(0) == right['split_sizes'].sum().item(), f"{right['pixel_values'].size(0)=},{right['split_sizes'].sum().item()=}"
    ...
    def pad_buffer(self, buffer):
        ...
        pad_split_sizes = torch.tensor([1] * num_pad_images, dtype=torch.long)
        ...
        buffer['split_sizes'] = torch.cat([buffer['split_sizes'], pad_split_sizes])
    ...
    def __iter__(self):
        ...
        while True:
            ...
            # if (len(buffer_max_len_list) > 0) or (len(buffer_list) > 0 and buffer_list[0]['pixel_values'].size(0) > self.num_images_expected) or (len(buffer_list) > 0 and buffer_list[0]['pixel_values'].size(0) == self.num_images_expected) or (len(buffer_list) > self.max_buffer_size):
